...
func bnEfficiency(size, height int, b, f, typ uint, goroutinesNumber, loopNumber int, reportPath string, t *testing.T) {
	var (
	...
	for i := 0; i < loopNumber; i++ {
		cuckooSupplementaryLine, cuckooFPSupplementaryLine, totalCosts, totalFPCount := parallelBNContainsAndReport(size, height, filter, b, f, typ, goroutinesNumber, i+1, loopNumber)
...
//
func parallelBNContainsAndReport(size, height int, bn birdsnest.BirdsNest, b, f, typ uint, goroutinesNumber, number, loopNumber int) (*charts.Line, *charts.Line, time.Duration, uint32) {
	...
	p, _ := ants.NewPoolWithFunc(goroutinesNumber, func(i interface{}) {
		parallelBNContains(size, height, bn, supplementaryContainsCosts, supplementaryFPCount, supplementaryHeights)
		wg.Done()
	})
	defer p.Release()
	now := time.Now()
...
func parallelBNContains(size, height int, bn birdsnest.BirdsNest, costs, fpCount []interface{}, heights []uint64) {
	for i := 0; i < size/height; i++ {
		var fp uint32
		// 查重
		now := time.Now()
